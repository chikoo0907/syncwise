...
export default function TimelineTracker() {
  ...
  const inProgressTimelines = timelines.filter(d => d.status === "in-progress").length;
  const completedTimelines = timelines.filter(d => d.status === "completed" || d.status === "delivered").length;
        ...
        <CardContent>
          {timelines.length === 0 ? (
            <div className="text-center text-gray-500 py-8">
              ...
              <p>No timeline items found. They will appear here once your company adds them.</p>
            </div>
          ) : (
